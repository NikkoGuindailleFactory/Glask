...
La plus simple consiste à créer un listener (comme j’aime à l’appeler) dans le loop et qui écoute littéralement l’état du pin. Mais cette méthode est limitée par le délai accordé dans le loop. C’est à dire que si le loop tourne à 100 ms, on a une précision à 100 ms. Ce qui est loin d’être fameux. On peut également diminuer ce délai, ce qui semble être radical mais qui a tendance à faire planter l’ESP et niveau consommation, ça doit se ressentir. Je ne suis pas expert là dedans malheureusement. C’est un peu lourd aussi comme façon de faire.
